...
async def library_search(message, client, mainmsg):
    ...
    if book_list_str != '':
        await mainmsg.edit(content=book + "의 검색 결과입니다.\n```" + book_list_str + '```' + '소장사항을 보길 원하는 도서의 번호를 입력해주세요.')
        ...
        await mainmsg.edit(content=book_rental_msg)
    else :
        await mainmsg.edit(content='검색 결과가 없습니다.')

async def library_collection(message, client, longmsg, mainmsg):
    longmsg.append(await message.channel.send("```동의대 콜랙션을 확인할 수 있습니다.\n원하는 번호를 선택해주세요.\n1. 베스트셀러\n2. 동의대권장도서\n3. 원북원부산도서\n4. 특성화(트렌드)도서\n5. 북큐레이션```"))
    ...
    if input_no == '1':
        ...
        mainmsg = await mainmsg.edit(content=collection_msg)
