...
async def search_summoner(message, riot_token, longmsg):
    if message.content.startswith("/검색 "):
        ...
        if res.status_code == 200:    # 올바른 소환사 명 입력시 서버상태 코드인 200출력
            ...
            longmsg.append(await message.channel.send(embed=embed, content=""))  # UserName 띄워쓰기 제거

        else:  # 존재하지 않는 소환사일떄, 없는 소환사 명은 오류코드 4?? 출력
            error = discord.Embed(title="존재하지 않는 소환사명입니다.\n다시 한번 확인해주세요.", color=0xFF9900)
            longmsg.append(await longmsg.channel.send(embed=error, content=""))
